    ...
    class AuthorizedNetGateway < Gateway
      ...
      def add_address(post, options)      
        ...
        if address = options[:billing_address] || options[:address]
          post[:address]    = address[:address1]    unless address[:address1].blank?
          post[:company]    = address[:company]     unless address[:company].blank?
          post[:phone]      = address[:phone]       unless address[:phone].blank?
          post[:zip]        = address[:zip]         unless address[:zip].blank?
          post[:city]       = address[:city]        unless address[:city].blank?
          
          if not address[:country].blank?
